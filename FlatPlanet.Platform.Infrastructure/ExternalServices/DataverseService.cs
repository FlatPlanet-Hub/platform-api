...
public sealed class DataverseService : IDataverseService
{
    ...
    public async Task<IEnumerable<EmployeeDto>> GetEmployeesAsync()
    {
        ...
        // fp_classification is a LOOKUP field (FK to a Classification entity), so it follows
        // the same _<name>_value pattern as activeclientofficer/activeclient. The OData
        // FormattedValue annotation gives the human-readable label (e.g. "Offsite", "Virtual"),
        // which RTW List uses to filter staff.
        const string query =
            "fp_employees?$select=fp_name,fp_employmentdate,fp_separationdate," +
            "fp_employmentstatus,_fp_classification_value," +
