...
{
    Task<ProjectResponse> CreateProjectAsync(Guid userId, string actorEmail, Guid companyId, string baseUrl, CreateProjectRequest request, string? ipAddress);
    ...
    Task DeactivateProjectAsync(Guid projectId, Guid userId, string actorEmail, string? ipAddress);
}
